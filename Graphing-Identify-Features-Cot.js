...
setColumns(["period", "phaseShift", "range", "verStretch", "vertShift", "equation"]);
...
for (let i = 0; i < rows; i++) {
  ...
  // Add quotes around the range string to handle the comma in CSV.
  ...
  // b = frequency, from period = pi / b => b = pi / period
  const b = periodDen;
  ...
  let equation = '';
  // Add the amplitude 'a' unless it's 1.
  if (a !== 1) {
    equation += a;
  }
  equation += `\\cot\\left[${b}\\left(x-${c}\\right)\\right]`;

  // Add the vertical shift 'd' if it's not zero.
  if (d > 0) {
    equation += `+${d}`;
  } else if (d < 0) {
    equation += `${d}`;
